...
use App\Mail\ScholarRequestStatusUpdate;
...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $inProgressApplications = Application::where('status', 'in_progress')->count();
    ...
    {
        $application = Application::with('user', 'adminNotes')->findOrFail($id);
        return view('admin.application-detail', compact('application'));
    }

    /**
     * Update application status and notify applicant when status changes
     */
    public function updateApplicationStatus(Request $request, $id)
    {
        ...
        if ($request->filled('admin_notes')) {
            ...
            $application->adminNotes()->save($note);
        }

        // Queue email notification if status changed
        if ($oldStatus !== $application->status) {
            $recipient = $application->email ?? ($application->user->email ?? null);
            if ($recipient) {
                Mail::to($recipient)->queue((new ApplicationStatusUpdate($application))->onQueue('emails'));
            }
        }
        ...
        $callback = function() use ($applications) {
            ...
            foreach ($applications as $application) {
                fputcsv($file, [
                    ...
                    $application->is_iba_indigene ? 'Yes' : 'No',
    ...
    public function exportSummaryReport()
    {
        ...
        $callback = function() {
            ...
            fputcsv($file, ['Under Review Applications', Application::where('status', 'under_review')->count()]);
            fputcsv($file, ['In Progress Applications', Application::where('status', 'in_progress')->count()]);
    ...
    {
        $request->validate([
            'status' => 'required|in:pending,under_review,in_progress,resolved,closed,rejected',
            'admin_notes' => 'nullable|string',
        ]);

        $scholarRequest = ScholarRequest::findOrFail($id);
        $oldStatus = $scholarRequest->status;
        $scholarRequest->update($request->only('status', 'admin_notes'));

        // Send queued email notification if status changed
        if ($oldStatus !== $scholarRequest->status && $scholarRequest->user && $scholarRequest->user->email) {
            Mail::to($scholarRequest->user->email)->queue((new ScholarRequestStatusUpdate($scholarRequest))->onQueue('emails'));
        }
    ...
    /**
     * Toggle admin note checked status
     */
    public function toggleAdminNote($noteId)
    {
        $note = AdminNote::findOrFail($noteId);
        $note->is_checked = !$note->is_checked;
        $note->save();

        return redirect()->back()->with('success', 'Note status updated!');
    }

    /**
     * Delete admin note
     */
    public function deleteAdminNote($noteId)
    {
        $note = AdminNote::findOrFail($noteId);
        $note->delete();
