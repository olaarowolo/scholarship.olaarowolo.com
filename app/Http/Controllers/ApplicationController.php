    ...
    public function submit(Request $request)
    {
        try {
            $validatedData = $request->validate([
                ...
                'admissionLetter' => 'nullable|file|mimes:pdf,jpg,jpeg,png|max:5120',
            ]);
        ...
        try {
            ...
            $admissionLetterPath = $request->file('admissionLetter') ? $request->file('admissionLetter')->store('applications/admission_letters', 'public') : null;
            ...
            $application = Application::create([
                ...
                'is_iba_indigene' => $request->isIndigene === 'Yes',
                'address' => $request->address,
                'institution' => $request->institution,
                'course' => $request->course,
                'level' => $request->level,
                ...
                'jamb_year' => $request->jambYear,
                'jamb_result_path' => $jambResultPath,
                'waec_result_path' => $waecResultPath,
                'indigene_certificate_path' => $indigeneCertPath,
                'admission_letter_path' => $admissionLetterPath ?? null,
                'status' => 'submitted',
                'notes' => $notes,
            ]);
            ...
            Mail::to(config('mail.admin.to'))->cc(config('mail.admin.cc'))->send(new ApplicationSubmittedAdmin($application, $applicantUser));
            // Log message to tray for admin
            $adminUser = \App\Models\User::where('email', config('mail.admin.to')[0])->first();
            if ($adminUser) {
                \App\Models\Message::create([
