...
return new class extends Migration
{
    ...
    public function down(): void
    {
        ...
        if (Schema::getConnection()->getDriverName() === 'sqlite') {
            ...
            DB::statement('INSERT INTO scholar_requests_temp (id, user_id, request_type, subject, description, priority, status, admin_notes, created_at, updated_at)
                          SELECT id, user_id, request_type, subject, description, priority, status, admin_notes, created_at, updated_at FROM scholar_requests WHERE status != "rejected"');

            Schema::drop('scholar_requests');
            Schema::rename('scholar_requests_temp', 'scholar_requests');
        } else {
            // For MySQL/PostgreSQL
            DB::statement("ALTER TABLE scholar_requests MODIFY COLUMN status ENUM('pending', 'in_progress', 'resolved', 'closed') DEFAULT 'pending'");
        }
    }
};
