...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <p>Your scholar request status has been updated to: <strong>{{ ucfirst($request->status) }}</strong></p>
            <div class="info-box">
                <strong>Request ID:</strong> #{{ $request->id }}<br>
                <strong>Subject:</strong> {{ $request->subject }}<br>
                <strong>Type:</strong> {{ ucfirst(str_replace('_', ' ', $request->request_type)) }}<br>
                <strong>Priority:</strong> {{ ucfirst($request->priority) }}<br>
                <strong>Status:</strong> {{ ucfirst($request->status) }}<br>
                <strong>Last Updated:</strong> {{ $request->updated_at->format('F d, Y \a\t h:i A') }}
            </div>
            <h3>Your Request:</h3>
            <p style="background: #f9fafb; padding: 15px; border-radius: 4px;">{{ $request->description }}</p>
            @if($request->admin_notes)
                <h3>Admin Notes:</h3>
                <p style="background: #fef3c7; padding: 15px; border-radius: 4px; border-left: 4px solid #f59e0b;">{{ $request->admin_notes }}</p>
            @endif
            <div style="text-align: center;">
                <a href="{{ url('/scholar/requests/my-requests') }}" class="button">View My Requests</a>
            </div>
