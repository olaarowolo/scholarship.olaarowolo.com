                                ...
                                <div class="bg-white border rounded-lg p-4 shadow-sm">
                                    <div class="flex items-center justify-between">
                                        <div class="min-w-0">
                                            <div class="flex items-center space-x-3">
                                                <div class="flex-shrink-0 h-10 w-10 bg-purple-100 rounded-full flex items-center justify-center">
                                                    <span class="text-purple-600 font-semibold">{{ strtoupper(substr($user->name, 0, 2)) }}</span>
                                                </div>
                                                <div class="min-w-0">
                                        ...
                                        <div class="ml-3 flex-shrink-0 text-right space-y-2">
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold {{ $user->role == 'admin' ? 'bg-purple-100 text-purple-800' : ($user->role == 'scholar' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800') }}">{{ ucfirst(str_replace('_',' ', $user->role ?? 'user')) }}</span>
                                    ...
                                    <div class="mt-4 flex items-center justify-between">
                                        <form method="POST" action="{{ route('admin.users.update-role', $user->id) }}" class="w-full">
                                            ...
                                            <div class="flex items-center gap-3">
                                                <select name="role" onchange="if(confirm('Change role for {{ addslashes($user->name) }}?')) { this.form.submit(); }"
                                                    ...
                                                    <option value="review_team" {{ $user->role == 'review_team' ? 'selected' : '' }}>Review Team</option>
                                                </select>
                                                <a href="{{ route('admin.users') }}#" class="text-sm text-gray-500">Manage</a>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @forelse ($users as $user)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $user->role == 'admin' ? 'bg-purple-100 text-purple-800' : ($user->role == 'scholar' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800') }}">{{ ucfirst(str_replace('_', ' ', $user->role ?? 'applicant')) }}</span>
                                            </td>
