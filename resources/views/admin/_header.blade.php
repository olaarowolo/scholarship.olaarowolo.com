<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6 mb-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div class="min-w-0">
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900 truncate">{{ $title ?? 'Admin' }}</h1>
            @if(!empty($subtitle))
                <p class="mt-1 text-sm md:text-lg text-gray-600 truncate">{{ $subtitle }}</p>
            @endif
        </div>
        ...
        @else
            <nav class="text-sm" aria-label="Breadcrumb">
                <ol class="list-reset flex items-center space-x-2 text-gray-500">
                    ...
                    <li class="text-gray-700">{{ $breadcrumbTitle ?? ($title ?? 'Overview') }}</li>
                </ol>
            </nav>
        @endif
    </div>
